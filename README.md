...
Another design pattern we used is the MVC. Model - The PlayerMovement class is responsible for storing the "state" related to players, including input, and exposing only changes to the state through events. PlayerMovement has no references to Pigeon, Lion, Door, or Bush. As for input, this is managed in Update() and ClickedSystem(): Key Presses (for example, C for pigeon call) and Ray Cast Clicks cause events to be raised such as OnPigeonCallRequested?.Invoke(), doorClickedOn?.Invoke(), and birdClickedOn?.Invoke(). When a collision occurs with the lion, OnLoseLife?.Invoke() is invoked (line 211). Therefore, the model owns the state and "something happened" signals; however, the model itself has no knowledge of how the UI or NPCs will respond to these signals. View - Views are the components used to display or hide information in response to those signals. Pigeon is a View: In Start() (line 39), it subscribes to birdClickedOn and determines if the dialogue box should be active (dialougeBox.SetActive) and what the hintText should be. The game-over UI in the GameController (i.e. _gameOverUI.SetActive(true) in CheckGameOver()) is another View. Door modifies its own transform and text; BushHidingZone is the "view" of the bush as both an interactive zone (trigger volume) and as providing feedback to the model about the input it received from the zone (by calling SetHidden). Controller - Controllers listen to the model's signals (events) and determine how they want to react to them and therefore drive Views or other behavior. PigeonCallController listens for Player.OnPigeonCallRequested (line 8) and in the event handler (lines 17-29), it simply determines whether to call either CurrentPigeon.Enable() or Disable() — it doesn't receive any raw input or have knowledge of any pigeon state. GameController listens for Player.OnLoseLife and causes the game-over UI and time scale to be enabled. Door listens for Player.doorClickedOn (line 26) and runs doorLogic(); it then raises correctKey, which is used by Pigeon to move to the next line of dialogue. Lion listens for Player.OnPlayerHidden and, when the player is hidden, it will switch to walking rather than chasing. Therefore, each Controller responds to one or more model signals (events) and alters either a View or the state of a NPC; the model remains completely unaware of the Controllers.

Another promeinet design pattern we used was the use of singleton pattern. Our game relies heavily on sending information between scripts, like when the player opens the first door or gets an answer wrong. Our group used a GameController script to act as a locator that could send information between all the scripts in the game. Then, some scripts have events that will send a message to the game when an action happens. For example, in the door script, there are two events that are dependent on if the player opens the door with a correct item. If they succesfully use a key to open the door, then a message called "correctKey" is sent out, where any other script can to subscribe to the event and trigger a function. Without the use of a locator or singletone pattern, sending information amongst the script would have been much more challenging and would have required more tedious hard coding, making the development process even more challeging. Other examples include events for the player clicking objects, getting a question right or wrong, and even several events in the gamecontroller that tell when teh game is over.
...
### Romarick Anderson 
After the check in, my main contributions involved making sure the door, puzzle system, bird, and player mouse system were functional.

For the mouse system, i had to re create how the player interacts with objects and npc. In the check in, most of the objects used there own OnMouseDown function to determine if they were clicked on. However, as I started to work more on expanding the door and bird funcationality, i realized that the system was not good with the scailing of the game. For this, i added several events and the ClickedSystem function that would send out a ray from the player camera and check if the player clicked the left mouse button while facing an object. If the player did this, then object based events would be sent out for other scripts to subscribe too in order to trigger other functions.

For the door, i added several functions and systems that add longer playability to the game. In the check in, the player would only be able to interact with the door once before it disappeared. Now, the door changes locations once the player uses a key once on door, and will remove the current key from the player inventory. This is done through the CheckDoorIndex() and the update DoorLogic() function that will iteriate through 3 seperate doors. Additonally, I created the CheckGameOver() function that would stop and show the winText once they opened three seperate doors, creating an end goal for the player. I was also able to create events for if the player had a key to open the door, which was the correctKey and wrongKey events.

For the bird, i expanded on the dialougue options by using scriptables with string list. In the check in, i was simply using 2 string outputs that would change once in the dialogue UI screen. However, i overhaulded the pigeon data script and added more progressive dialogue on pigeon, that gives more context on the game and what the players quest. This was all done through the update pigeon script, which and AdvanceDialogue function(). I also added the SpawnFinalKey() function that will spawn the last when the player opens 2 doors. 

A new feature of the game is the puzzle system, that is a game object that will make a UI pop up with a question if the player clicks on it. Then, a question with two answers will appear, making the player choose an answer that could be correct or false. This is done through the PuzzleLogic script that was built for the Puzzle and PuzzleUI gameobjects.
...
We keep all the input data inside the player movement model. Inside the update() function, when the input is pressed by calling Input.GetKeyDown(KeyCode.C), we only fire an event.

There is another controller called PigeonCallController which subscribes to that event inside the start() function. And it unsubscribes inside the OnDestroy() function. After the event has fired, it retrieves the pigeon from GameController.Instance.CurrentPigeon. It then checks the state of the game object of the pigeon. If it's currently visible (active), it calls Disable(); otherwise it calls Enable(). So this "call" logic is contained within the controller; the pigeon is simply a view showing/hiding itself by toggling its gameObject.activeStatus().

In the awake() function, GameController finds the pigeon in the scene using the true version of FindObjectOfType() and stores it into the variable CurrentPigeon. Add a PigeonCallController to your manager GameObject (i.e. the same GameObject you have already added GameController to). Because there is no reference to an inspector field anywhere in the code - everything is set up via the singleton and events.

Stop hunting (lion stops attacking once player is in a bush).

Goal: When the player enters a bush, the lion should stop being aggressive and go back to roaming around. As soon as the player exits the bush, the lion should continue behaving normally - i.e., the lion does not need to be forced to go back to a particular state.

I created a new conceptual model in our code: "the player is hidden." The player movement model now contains an IsHidden property along with two new events, OnPlayerHidden and OnPlayerRevealed. Also, it contains a method SetHidden(bool hidden) which only modifies state when it is changed and then invokes the correct event. Therefore, the model maintains the "hidden" state and provides notifications to any interested parties; however, it remains completely independent of both the bush and the lion.

Each bush will contain a new script called BushHidingZone. Within the OnTriggerEnter function of BushHidingZone, it checks to see if the current collider entered is tagged with the player tag. If it is, then it will call GameController.Instance.Player.SetHidden(true). Within the OnTriggerExit function of BushHidingZone, it will call SetHidden(false). Thus, the bush is responsible for triggering the "hidden" state; the model remains decoupled from the bush prefab.

The lion subscribes to OnPlayerHidden and OnPlayerRevealed inside the OnEnable() function and unsubscribes inside the OnDisable() function. When OnPlayerHidden fires, if the lion is currently in a pursuing state, it calls ChangeState(LionState._wandering) so it ceases to chase the player. There is nothing done inside the OnPlayerRevealed event because we do not want to change the lion's state when the player leaves the bush. Instead, the lion's current behavior is to start chasing the player again as soon as it sees them: we modified the lion to treat a hidden player as invisible. At the beginning of the CanSeePlayer() function, we check if GameController.Instance.Player.IsHidden and return false if it is. We also modified the condition that begins the lion's pursuit to include !Player.IsHidden. So, the lion never "sees" a player that is hidden and therefore never pursues a player that is hidden.
